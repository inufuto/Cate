...
internal class WordRegister(int id, Cate.ByteRegister high, Cate.ByteRegister low)
    : Cate.WordRegister(id, high.Name + low.Name)
{
    ...
    public static List<Cate.WordRegister> Registers = [Hl, De, Bc];
    ...
    private void LoadIndirect(Instruction instruction, Cate.WordRegister pointerRegister)
    {
        if (Equals(pointerRegister, this)) {
    ...
    protected virtual void StoreIndirect(Instruction instruction, Cate.WordRegister pointerRegister)
    {
        using (ByteOperation.ReserveRegister(instruction, ByteRegister.A)) {
            Debug.Assert(Low != null);
            Debug.Assert(High != null);
